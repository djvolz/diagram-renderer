...
@click.group()
@click.version_option(version="0.1.0", prog_name="diagram-render")
def cli():
    """
...
@click.option('--detect-only', is_flag=True, 
              help='Only detect diagram type, don\'t render')
...
def render(input_file, output, detect_only, verbose):
    ...
    Examples:
    \b
        diagram-render render flowchart.mmd
        diagram-render render diagram.txt -o output.html
        diagram-render render --detect-only input.md
    """
    try:
        # Read input file
        if verbose:
...
def quick(diagram_code, diagram_type, output, verbose):
    ...
    Examples:
    \b
        diagram-render quick "graph TD; A-->B"
        diagram-render quick "@startuml; A->B; @enduml" --type plantuml
        diagram-render quick "digraph G { A -> B; }" -o my-graph.html
    """
    try:
        renderer = DiagramRenderer()
...
def dashboard():
    ...
    try:
        ...
        subprocess.run(["uv", "run", "streamlit", "run", str(dashboard_path)], check=True)
    except subprocess.CalledProcessError as e:
        click.echo(f"❌ Error running Streamlit: {e}", err=True)
        sys.exit(1)
    except FileNotFoundError:
        click.echo("❌ Error: Streamlit not found. Make sure it's installed with 'uv install'", err=True)
        sys.exit(1)
